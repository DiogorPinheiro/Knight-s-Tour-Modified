...
let matriz = make_matrix n n 0 
let () = matriz.(x_init).(y_init) <- 1 
let () =
    if peoes <> 0 then
    for i = 0 to (peoes - 1)  do
...
let rec resultado x y n saltos matriz possivel xp yp =
    ...
    let rec newFor cont x y n saltos matriz possivel xp yp =    
        if(cont < 8) then 
...
let () =         
        if ( resultado x_init y_init n 2 matriz possivel xp yp ) = true then
            let () = printf "YES\n" in ()
        else
            let () = printf "NO\n" in () 